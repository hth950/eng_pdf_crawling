...
for p_id, p_value in data.items():
    if len(p_value) > 1:
        print(f"Warning: {p_id} has more than one top-level key. Skipping.")
    for top_key, top_value in p_value.items():
        if (
            "심화" in top_key
            or "다락원" in top_key
            or not any(kw in top_key for kw in valid_keywords)
        ):
            continue
        ...
        for cand in publisher_candidates:
            if publisher_info.startswith(cand):
                ...
                author = publisher_info[
                    len(cand) :
                ]  # 후보 이름 길이 이후의 문자열이 저자
                break
        ...
        new_inner["세부과목"] = (
            subject
            if top_key.startswith("고등") or top_key.startswith("공통영어")
            else ""
        )
        ...
        # 최종 결과에 추가
        new_data[p_id] = new_inner

# 결과를 새로운 JSON 파일로 저장
output_file = r"./2022_results_organize.json"
